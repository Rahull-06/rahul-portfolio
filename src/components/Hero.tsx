...
export default function Hero() {
    ...
    return (
        <>
            ...
            <div
                ref={cursorRef}
                className="fixed w-2 h-2 bg-lime rounded-full pointer-events-none z-[9999] -translate-x-1/2 -translate-y-1/2 shadow-[0_0_12px_rgba(200,241,53,0.5)] mix-blend-difference transition-[width,height] duration-200"
            />
            <div
                ref={ringRef}
                className="fixed w-8 h-8 border border-lime/40 rounded-full pointer-events-none z-[9998] -translate-x-1/2 -translate-y-1/2 transition-[width,height] duration-300"
            />

            {/* Hero grid */}
            <div className="min-h-screen grid grid-cols-2 relative overflow-hidden">
                {/* LEFT */}
                <div className="flex flex-col justify-center px-16 pt-36 pb-16 relative z-10">
                    {/* Eyebrow */}
                    <div className="flex items-center gap-3 mb-8 animate-fadeUp" style={{ animationDelay: "0.1s" }}>
                    ...
                    {/* Name */}
                    <h1
                        className="font-serif text-[6rem] font-black leading-[0.92] tracking-[-4px] animate-fadeUp"
                    ...
                    {/* Tagline */}
                    <p
                        className="font-serif text-2xl font-bold mt-6 mb-2 leading-tight animate-fadeUp"
                    ...
                    {/* Role */}
                    <div
                        className="flex items-center gap-3 mb-6 animate-fadeUp"
                        style={{ animationDelay: "0.4s" }}
                    >
                        <div className="w-2 h-2 bg-orange rounded-full animate-blink flex-shrink-0" />
                        <span className="font-mono text-gray2 text-xs tracking-widest">
                    ...
                    {/* Desc */}
                    <p
                        className="text-gray2 text-sm leading-loose max-w-md mb-10 font-light animate-fadeUp"
                    ...
                    >
                        <a
                            href="#projects"
                            className="font-mono text-xs tracking-widest uppercase px-8 py-3 bg-lime text-ink font-medium transition-all duration-300 hover:scale-105 hover:shadow-[0_0_24px_rgba(200,241,53,0.4)]"
                        >
                            View Projects →
                        </a>
                        <a
                            href="#contact"
                            className="font-mono text-xs tracking-widest uppercase px-8 py-3 border border-border2 text-gray2 transition-all duration-300 hover:border-purple hover:text-purple hover:scale-[1.03]"
                        >
                            Let&apos;s Talk
                        </a>
                    </div>
            ...
            <div className="flex border-t border-border">
                {stats.map((s) => (
                    <div
                        key={s.label}
                        className="flex-1 px-8 py-5 border-r border-border last:border-r-0 bg-ink/90 backdrop-blur-md hover:bg-ink2 transition-colors duration-300"
                    >
