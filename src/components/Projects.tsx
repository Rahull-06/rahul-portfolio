...
export default function Projects() {
    ...
    return (
        <section id="projects" ref={ref} className="px-16 py-28">
            <div className="max-w-[1240px] mx-auto">
                ...
                <div className="flex flex-col gap-px border border-border">
                    ...
                    {/* Coming soon */}
                    <div className="px-10 py-8 flex items-center gap-4 border-t border-dashed border-border2">
...
function ProjectRow({ project: p, delay }: { project: Project; delay: number }) {
    return (
        <div
            className="reveal grid grid-cols-[70px_1fr_180px] border-b border-border last:border-b-0 transition-colors duration-300 hover:bg-ink2 relative group"
        ...
        >
            ...
            {/* Number */}
            <div className="p-8 pt-10 border-r border-border flex items-start justify-center">
            ...
            <div className="p-8">
                <div className="flex gap-2 mb-3 flex-wrap">
                    {p.badges.map((b) => (
                        <span
                            key={b.text}
                            className={`font-mono text-[0.58rem] px-2.5 py-0.5 border uppercase tracking-widest ${b.live
                ...
                <h3 className="font-serif text-2xl font-bold text-white mb-3 leading-tight tracking-tight">
                    {p.title}
                </h3>
                <p className="text-gray2 text-sm leading-loose font-light mb-5">{p.desc}</p>

                <ul className="space-y-2 mb-6">
                    {p.bullets.map((b, i) => (
                        <li key={i} className="text-[0.84rem] text-gray2 pl-5 relative leading-relaxed font-light">
                ...
                {/* Impact */}
                <div className="flex gap-0 pt-4 border-t border-border mb-5 flex-wrap">
                    {p.impact.map((item) => (
                        <div key={item.label} className="flex-1 min-w-[110px] pr-4">
                            <span className="font-serif text-lg font-bold text-orange block">{item.val}</span>
                            <span className="font-mono text-[0.58rem] text-gray uppercase tracking-widest">{item.label}</span>
                        </div>
                    ))}
                </div>
            ...
            {/* Actions */}
            <div className="p-6 border-l border-border flex flex-col justify-center gap-3">
                {p.links.map((l) => (
                    <a
                        key={l.label}
                        href={l.href}
                        ...
                        className={`flex items-center justify-center gap-2 font-mono text-[0.62rem] px-3 py-2.5 border uppercase tracking-widest transition-all duration-300 whitespace-nowrap hover:scale-[1.04] ${l.primary
